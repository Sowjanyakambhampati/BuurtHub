...
function UserDashboard({session}) {
    ...
    useEffect(() => {
        const fetchRegisteredEvents = async () => {
            try {
                const response = await axios.get(`https://community-forum-backend.vercel.app/event/registeredevents/${user.id}`);
    ...
    useEffect(() => {
        const fetchUserProducts = async () => {
            try {
                const response = await axios.get(`https://community-forum-backend.vercel.app/product/productowner/${user.id}`);
    ...
    useEffect(() => {
        const fetchUserReservedProducts = async () => {
            try {
                const response = await axios.get(`https://community-forum-backend.vercel.app/product/reservedproducts/${user.id}`);
    ...
    useEffect(() => {
        const fetchUserFavouriteProducts = async () => {
            try {
                const response = await axios.get(`https://community-forum-backend.vercel.app/product/favouriteproducts/${user.id}`);
    ...
    useEffect(() => {
        const fetchUserPosts = async () => {
            try {
                const response = await axios.get(`https://community-forum-backend.vercel.app/posts/postauthor/${user.id}`);
    ...
    const handleEditSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.put(`https://community-forum-backend.vercel.app/product/${editProduct._id}`, editProduct);
    ...
    const handlePostEditSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.put(`https://community-forum-backend.vercel.app/posts/postauthor/${editPost._id}`, editPost);
    ...
    const handleDelete = async (productId) => {
        try {
            await axios.delete(`https://community-forum-backend.vercel.app/product/${productId}`);
    ...
    const handlePostDelete = async (postId) => {
        try {
            await axios.delete(`https://community-forum-backend.vercel.app//posts/postauthor/${postId}`);
                                    ...
                                    {editProduct && editProduct._id === product._id ? (
                                        <form onSubmit={handleEditSubmit} className="space-y-4">
                                            <div className="form-group">
                                                <label
                                                    className="block text-sm font-medium text-firstcolor">Name:</label>
                                                <input
                                                    name="productName"
                                            ...
                                            <div className="form-group">
                                                <label
                                                    className="block text-sm font-medium text-firstcolor">Description:</label>
                                                <textarea
                                                    name="description"
                                            ...
                                            <div className="form-group">
                                                <label
                                                    className="block text-sm font-medium text-firstcolor">Price:</label>
                                                <input
                                                    name="price"
                                                    type="number"
                                            ...
                                            <div className="form-group">
                                                <label
                                                    className="block text-sm font-medium text-firstcolor">Category:</label>
                                                <select
                                                    name="category"
                                            ...
                                            <div className="form-group">
                                                <label
                                                    className="block text-sm font-medium text-firstcolor">Condition:</label>
                                                <select
                                                    name="condition"
                                        ...
                                        <div className="flex flex-col h-full">
                                            ...
                                            <p className="flex text-firstcolor text-left mb-4">
                                                <BsFillInfoCircleFill
                                                    className="m-1 text-thirdcolor"/> {product.description}
                                            </p>
                                            <p className="flex text-firstcolor mb-2 text-left">
                            ...
                            reservedProducts.map(product => (
                                <div key={product._id} className="bg-white p-4 rounded-lg shadow-md">
                                    ...
                                    <p className="flex text-firstcolor mb-2 text-left"><IoIosPricetags
                                        className="m-1 text-thirdcolor"/> € {product.price}.00</p>
                                    <p className="flex text-firstcolor mb-4"><BsFillInfoCircleFill
                                        className="m-1 text-thirdcolor"/>{product.description}</p>
                                    <p className="flex text-firstcolor mb-2 text-left"><MdCategory
                                        className="m-1 text-thirdcolor"/>{product.category}</p>
                                    <p className="flex text-firstcolor mb-2 text-left"><TbBox
                                        className="m-1 text-thirdcolor"/>{product.condition}</p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        {favouriteProducts.length > 0 ? (
                            favouriteProducts.map(product => (
                                <div key={product._id} className="bg-white p-4 rounded-lg shadow-md">
                                    <Link to={`/all-products/city/${selectedCity}/product/${product._id}`}
                                          state={{session}}
                                        ...
                                        <p className="flex text-firstcolor mb-2 text-left"><IoIosPricetags
                                            className="m-1 text-thirdcolor"/> € {product.price}.00</p>
                                        <p className="flex text-firstcolor mb-4 text-left"><BsFillInfoCircleFill
                                            className="m-1 text-thirdcolor"/>{product.description}</p>
                                        <p className="flex text-firstcolor mb-2 text-left"><MdCategory
                                            className="m-1 text-thirdcolor"/>{product.category}</p>
                                        <p className="flex text-firstcolor mb-2 text-left"><TbBox
                ...
                <section className="mb-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        {userPosts.length > 0 ? (
                            userPosts.map(post => (
                                <div key={post._id}
                                     className="bg-white p-4 rounded-lg shadow-md flex flex-col h-full relative">
                                    {editPost && editPost._id === post._id ? (
                                        <form onSubmit={handlePostEditSubmit} className="space-y-4 flex-grow">
                                            <div className="form-group">
                                                <label
                                                    className="block text-sm font-medium text-firstcolor">Title:</label>
                                                <input
                                                    name="title"
                                            ...
                                            <div className="form-group">
                                                <label
                                                    className="block text-sm font-medium text-firstcolor">Content:</label>
                                                <textarea
                                                    name="content"
